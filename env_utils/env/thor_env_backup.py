...
class ThorEnv(Controller):
    ...
    def restore_scene(self, object_poses, object_toggles, dirty_and_empty):
        ...
        #     for obj_tog in object_toggles:
        #         super().step(dict(
        #             action='SetObjectStates',
        #             SetObjectStates={'objectType': obj_tog['objectType'], 'stateChange': 'toggleable', 'isToggled': obj_tog['isOn']},
        #             forceAction=True,
        #         ))
    ...
    def look_angle(self, angle, render_settings=None):
        ...
        '''
        Valid ways to call "TeleportFull" action:
            Void TeleportFull(Single x, Single y, Single z, Vector3 rotation, Single horizon, Boolean standing, Boolean forceAction = False)
            Void TeleportFull(Vector3 position, Vector3 rotation, Single horizon, Boolean standing, Boolean forceAction = False)
        '''
        teleport_action = {
            'action': 'TeleportFull',
            'rotation': rotation,
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False):
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            if debug:
                ...
                # since we're using headless server (opencv-python-headless)
